...
const API_BASE = process.env.VIGILE_API_URL || "https://api.vigile.dev";
...
server.tool(
  ...
  {
    name: z.string().describe("MCP server name or npm package name (e.g., '@anthropic/mcp-server-filesystem')"),
  },
  async ({ name }) => {
...
server.tool(
  ...
  {
    name: z.string().describe("Agent skill name (e.g., 'react-component-builder')"),
  },
  async ({ name }) => {
...
server.tool(
  ...
  {
    content: z.string().describe("The raw text content to scan"),
    file_type: z.string().optional().describe("File type: skill.md, claude.md, cursorrules, mdc-rule (default: skill.md)"),
    name: z.string().optional().describe("Optional name for the scan result"),
  },
...
server.tool(
  ...
  {
    query: z.string().describe("Search query (e.g., 'filesystem', 'database', 'code execution')"),
